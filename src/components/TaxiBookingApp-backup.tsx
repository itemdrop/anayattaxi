    ...
    const initMap = async () => {
      try {
        ...
        if (!apiKey || apiKey === 'YOUR_GOOGLE_MAPS_API_KEY' || apiKey === 'your_google_maps_api_key_here') {
          console.warn('Google Maps API key not configured. Map will show placeholder.');
          // Show a placeholder or demo mode
          if (mapRef.current) {
            mapRef.current.innerHTML = `
              <div class="flex items-center justify-center h-full bg-gray-100 border-2 border-dashed border-gray-300 rounded-lg">
                <div class="text-center p-6">
                  <div class="text-4xl mb-2">🗺️</div>
                  <p class="text-gray-600 text-sm font-medium">Interactive Map</p>
                  <p class="text-gray-500 text-xs mt-1">Configure Google Maps API to enable</p>
                </div>
              </div>
            `;
          }
        ...
        // Use the traditional callback approach for better compatibility
        loader.load().then(() => {
        ...
        if (mapRef.current) {
          mapRef.current.innerHTML = `
            <div class="flex items-center justify-center h-full bg-red-50 border-2 border-red-200 rounded-lg">
              <div class="text-center p-6">
                <div class="text-4xl mb-2">⚠️</div>
                <p class="text-red-600 text-sm font-medium">Map Loading Failed</p>
                <p class="text-red-500 text-xs mt-1">Check your internet connection</p>
              </div>
            </div>
          `;
        }
        
        setLocationError('Map failed to load. Location services may be limited.');
      }
    };
  ...
  const getCurrentLocation = async () => {
    ...
    try {
      ...
      if (map) {
        ...
        map.setZoom(16); // Closer zoom for better accuracy
        ...
        const infoWindow = new window.google.maps.InfoWindow({
          content: `<div style="padding: 8px;"><strong>📍 Your Location</strong><br/>${address}</div>`
        });
          ...
          <div className="bg-gray-50 p-8 border-r border-gray-200">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button
                  onClick={() => {
                    // Demo location for testing (San Francisco)
